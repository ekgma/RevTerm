...
public class CFGUtil
{
    ...
    public static void weakestPreCondition(Vector<Transition> path,Putinar putinar,int counter)  //NOTE: for C-Integer programs this is completely fine but for general T2 transition systems it might have problems
    {
        ...
        for(int i=path.size()-1;i>=0;i--)
        {
            ...
            if(counter==0)
            {
                putinar.addPredicate(t.nondetGuard.deepCopy());
                putinar.addPredicate(t.detGuard.deepCopy());
            }
            for(int j=0;j<t.varName.size();j++)
            {
                String var=t.varName.elementAt(j);
                Polynomial upd=t.update.elementAt(j);
                putinar.replaceVarWithPoly(var,upd);
            }
