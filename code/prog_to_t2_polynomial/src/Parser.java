...
//        System.err.println("parseBexpr: "+beginIndex+"---"+endIndex);
        ...
        if (ors.size() > 2)
        {
            for (int i = 1; i < ors.size(); i++)
            {
                ...
                cur.guard = PolynomialPredicate.disjunct(cur.guard, ch.guard);
            }
            return cur;
        }
        if (ands.size() > 2)
        {
            for (int i = 1; i < ands.size(); i++)
            {
                ...
                cur.guard = PolynomialPredicate.conjunct(cur.guard, ch.guard);
            }
            return cur;
        }
        ...
        if (getToken(beginIndex).equals("!"))
        {
            ...
            cur.guard = PolynomialPredicate.negate(ch.guard,0);
            return cur;
        }
...
//        System.err.println("parseLiteral:"+ beginIndex+"---"+endIndex);
        ...
        else if (type == 2) // left > right   ->   left -right >=eps   ->   left - right -eps >=0
        {
            ...
            lc.minus(new Polynomial(prog_to_t2.eps));  // left - right - eps
            PolynomialPredicate lp = new PolynomialPredicate();
            lp.add(lc);
            cur.guard.add(lp);
        }
        else if (type == 3) //left < right  -->   right - left > eps   -->   right - left -eps >=0
        {
            ...
            lc.minus(new Polynomial(prog_to_t2.eps));  // right - left - eps
            PolynomialPredicate lp = new PolynomialPredicate();
            lp.add(lc);
            cur.guard.add(lp);
        }
        ...
        else if (type == 5)  // left != right  -->  left - right -1 >=0  or right - left -1 >=0
        {
            Polynomial lc = right.expr.deepCopy();
            lc.minus(left.expr);
            lc.minus(new Polynomial(prog_to_t2.eps));

            Polynomial lc2 = left.expr.deepCopy();
            lc2.minus(right.expr);
            lc2.minus(new Polynomial(prog_to_t2.eps));
...
//        {
//            Polynomial lc=left.expr.deepCopy(),lc2=right.expr.deepCopy();
//            lc.minus(right.expr);
//            lc2.minus(left.expr);
//            PolynomialPredicate lp=new PolynomialPredicate();
//            lp.add(lc);
...
//            lc.minus(new Polynomial(prog_to_t2.eps));
//            lc2.minus(new Polynomial(prog_to_t2.eps));
//            PolynomialPredicate lp=new PolynomialPredicate(),lp2=new PolynomialPredicate();
//            lp.add(lc);
//            lp2.add(lc2);
//            cur.guard.add(lp);
//            cur.guard.add(lp2);
